...
fn parse_v2_block(
    ...
    let (msg_start, msg_end, track_creation_order) = if sig == Some(OHDR_SIGNATURE) {
        ...
        // Since we don't store that here, we read until we hit a parse error;
        // the checksum occupies the final 4 bytes and won't form a valid
        // message record.
        //
    ...
    while pos + 4 <= msg_end && pos + 4 <= file_data.len() {
        ...
        if msg_type == MSG_NIL_V2 {
            // NIL is padding, not necessarily the end of the block.  HDF5
            // writers may place another continuation message after a NIL
            // record, so advance by its header and keep scanning.
            pos += hdr_size;
            continue;
        }
...
fn parse_v2_ochk_block(
    ...
    while pos + 4 <= msg_area_end {
        ...
        // Continuation blocks inherit the creation-order flag from the owning
        // OHDR. When set, each message has a two-byte creation-order field;
        // starting four bytes in that case shifts datatype bodies by two bytes.
        let hdr_size: usize = if track_creation_order { 6 } else { 4 };
        ...
        if msg_type == MSG_NIL_V2 {
            // NIL records can precede a later continuation in the same
            // chunk; treat them as padding and continue scanning.
            pos += hdr_size;
            continue;
        }
...
mod tests {
    ...
    #[test]
    fn test_parse_messages_v1_nil_is_padding() {
        ...
        data[16] = 0x01;
        data[18] = 0x00; // size = 0
        // Second message: NIL (type 0, all zeros) — padding
                         // data[24] = 0 (already)

        let msgs = parse_messages(&data, 0).unwrap();
        // The first 0-byte message might be collected. The all-zeros message
        // has type=0 (NIL) and is skipped.
        // The first message also has type=0x01 and size=0 so it's collected.
        // data[24] = type 0x00 = NIL.
        assert!(msgs.len() <= 1);
    }
