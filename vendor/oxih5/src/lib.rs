...
pub(crate) enum FileData {
    ...
    /// A bounded metadata window backed by a random-access source. The
    /// metadata window is used by the existing navigation parser; payload
    /// reads are delegated to the source-backed APIs below.
    Remote {
        metadata: std::sync::Arc<Vec<u8>>,
...
impl std::ops::Deref for FileData {
    ...
    fn deref(&self) -> &[u8] {
        match self {
            ...
            FileData::Remote { metadata, .. } => metadata.as_slice(),
        }
    }
}
...
impl std::fmt::Debug for FileData {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FileData::Remote { metadata, source } => write!(
                f,
                "Remote(metadata={}b, object={}b)",
                metadata.len(),
                source.len()
            ),
        }
...
impl File {
    ...
    /// Open an HDF5 file from a bounded metadata window and a random-access
    /// source. The source is consulted only by the source-aware dataset read
    /// methods; it is never materialized into the metadata window.
    pub fn open_with_source(
        metadata: Vec<u8>,
        source: std::sync::Arc<dyn ByteSource>,
    ) -> Result<Self, OxiH5Error> {
        if metadata.is_empty() {
            return Err(OxiH5Error::Format("remote metadata window is empty".into()));
        }
        if metadata.len() as u64 > source.len() {
            return Err(OxiH5Error::Format(format!(
                "remote metadata window ({} bytes) exceeds source length {}",
                metadata.len(),
                source.len()
            )));
        }
        Ok(File {
            data: FileData::Remote {
    ...
    /// Return whether this handle uses a random-access source rather than a
    /// complete local byte buffer or mmap.
    pub fn is_source_backed(&self) -> bool {
        matches!(self.data, FileData::Remote { .. })
    }

    /// Read dataset shape, dtype, attributes, and unlimited-dimension
    /// metadata without reading its payload.
    ///
    /// This is used while opening source-backed NetCDF-4 files: metadata
    /// discovery must not accidentally read every chunk in every variable.
    pub fn dataset_metadata(&self, path: &str) -> Result<Dataset, OxiH5Error> {
        let header_addr = self.resolve_dataset_header_addr(path)?;
        read_dataset_metadata_from_object_header(&self.data, header_addr, path)
    }

    /// Read dataset metadata directly from an object-header address without
    /// touching the dataset payload.
    pub fn dataset_metadata_at(&self, addr: u64) -> Result<Dataset, OxiH5Error> {
        if addr == u64::MAX {
            return Err(OxiH5Error::NotFound("undefined object reference".into()));
        }
        read_dataset_metadata_from_object_header(&self.data, addr, &format!("@{addr:#x}"))
    }
    ...
    pub fn dataset_slice(
        ...
        if let FileData::Remote { source, .. } = &self.data {
            return read_dataset_slice_from_source(
                &self.data,
                source,
                path,
                ranges,
    ...
    pub fn dataset_hyperslab(
        ...
        if let FileData::Remote { source, .. } = &self.data {
            if selection
                .iter()
                .any(|dimension| dimension.stride != 1 || dimension.block != 1)
            {
                return Err(OxiH5Error::NotImplemented(
                    "source-backed hyperslabs currently require unit stride and unit block"
                        .into(),
                ));
            }
            let ranges = selection
                .iter()
                .map(|dimension| {
                    let end = dimension.start.checked_add(dimension.count).ok_or_else(|| {
                        OxiH5Error::Format("source-backed hyperslab range overflow".into())
                    })?;
                    Ok(
                        usize::try_from(dimension.start)
                            .and_then(|start| usize::try_from(end).map(|end| start..end))
                            .map_err(|_| {
                                OxiH5Error::Format(
                                    "source-backed hyperslab exceeds addressable memory".into(),
                                )
                            })?,
                    )
                })
    ...
    pub fn info(&self) -> Result<FileInfo, OxiH5Error> {
        ...
        Ok(FileInfo {
            ...
            file_size: match &self.data {
                FileData::Remote { source, .. } => source.len(),
...
/// Parse only the metadata-bearing messages from a dataset object header.
fn read_dataset_metadata_from_object_header(
    file_data: &[u8],
    header_addr: u64,
    name: &str,
) -> Result<Dataset, OxiH5Error> {
    let messages = header::parse_messages(file_data, header_addr)?;
    let mut dataspace = None;
    let mut datatype = None;
    ...
    for msg in &messages {
        match msg.msg_type {
            ...
            0x0003 => datatype = Some(message::parse_datatype(&msg.data)?),
            0x000C => {
                if let Ok(attribute) = message::parse_attribute(&msg.data) {
    ...
    let dataspace = dataspace.ok_or_else(|| {
        OxiH5Error::Format(format!("no dataspace message in dataset '{name}'"))
    })?;
    let datatype = datatype.ok_or_else(|| {
        OxiH5Error::Format(format!("no datatype message in dataset '{name}'"))
    })?;

    Ok(Dataset {
        data: Vec::new(),
        shape: dataspace.dims.iter().map(|&dim| dim as usize).collect(),
        dtype: datatype.dtype,
        attributes,
        max_dims: dataspace.max_dims,
    })
}

/// Read a source-backed dataset slice. Metadata and chunk-index bytes come
/// from the bounded metadata window; compressed chunk payloads come directly
/// from the random-access source.
fn read_dataset_slice_from_source(
    file_data: &[u8],
    source: &std::sync::Arc<dyn ByteSource>,
    path: &str,
    ranges: &[std::ops::Range<usize>],
    ...
    let header_addr = resolve_dataset_header_addr_in_bytes(file_data, path)?;
    let messages = header::parse_messages(file_data, header_addr)?;
    let mut dataspace = None;
    let mut datatype = None;
    let mut layout = None;
    ...
    for msg in &messages {
        match msg.msg_type {
            ...
            0x0008 => layout = Some(message::parse_layout(&msg.data)?),
            0x000B => filter_pipeline = Some(message::parse_filter_pipeline(&msg.data)?),
            0x0005 => fill_value = message::parse_fill_value(&msg.data)?,
            0x000C => {
                if let Ok(attribute) = message::parse_attribute(&msg.data) {
                    attributes.push(attribute);
                }
    ...
    let dataspace = dataspace.ok_or_else(|| {
        OxiH5Error::Format(format!("no dataspace message in dataset '{path}'"))
    })?;
    let datatype = datatype.ok_or_else(|| {
        OxiH5Error::Format(format!("no datatype message in dataset '{path}'"))
    })?;
    let layout = layout.ok_or_else(|| {
        OxiH5Error::Format(format!("no layout message in dataset '{path}'"))
    })?;
    let dataset_dims = dataspace.dims.clone();
    let ranges_u64 = ranges
        .iter()
        .map(|range| {
            u64::try_from(range.start)
                .and_then(|start| u64::try_from(range.end).map(|end| start..end))
                .map_err(|_| OxiH5Error::Format("slice range exceeds u64".into()))
        })
        .collect::<Result<Vec<_>, _>>()?;
    let elem_size = on_disk_elem_footprint(&datatype.dtype, 8).ok_or_else(|| {
        OxiH5Error::NotImplemented(format!("dataset '{path}': element size not supported"))
    })?;
    let pipeline = filter_pipeline.unwrap_or_else(|| oxih5_core::FilterPipeline { filters: vec![] });

    let raw = match layout {
        oxih5_format::message::LayoutInfo::Chunked { .. } => {
            if ranges_u64.len() != dataset_dims.len() {
                return Err(OxiH5Error::Format(format!(
                    "dataset '{path}': {} ranges for {} dimensions",
                    ranges_u64.len(),
                    dataset_dims.len()
                )));
            }
            oxih5_format::chunked::read_chunked_slice_with_reader(
                file_data,
                &layout,
                &pipeline,
                &dataset_dims,
                ...
                Some(cache),
                source.len(),
                |offset, length| source.read(offset, length),
                |record| {
                    let length = usize::try_from(record.size).map_err(|_| {
                        OxiH5Error::Format(format!("dataset '{path}': chunk size overflows usize"))
                    })?;
                    let bytes = source.read(record.address, length)?;
                    if bytes.len() != length {
                        return Err(OxiH5Error::Format(format!(
                            "dataset '{path}': source returned {} bytes for chunk of {} bytes",
                            bytes.len(),
                            length
                        )));
                    }
        ...
        oxih5_format::message::LayoutInfo::Contiguous {
            data_address,
            ..
        } => {
            oxih5_format::gather_hyperslab_contiguous_with_reader(
                &|offset, length| source.read(offset, length),
                data_address,
                &dataset_dims,
                &oxih5_format::Hyperslab {
                    dims: ranges_u64
                        .iter()
                        .map(|range| oxih5_format::DimSelection::contiguous(range.start..range.end))
                        .collect(),
                },
                elem_size,
            )?
        }
        oxih5_format::message::LayoutInfo::Compact { data } => {
            oxih5_format::gather_hyperslab_contiguous(
                &data,
                &dataset_dims,
                &oxih5_format::Hyperslab {
                    dims: ranges_u64
        ...
        oxih5_format::message::LayoutInfo::VirtualDataset { .. } => {
            return Err(OxiH5Error::NotImplemented(format!(
                "dataset '{path}': source-backed virtual datasets are not supported"
            )))
        }
    };
...
/// Resolve a dataset header using the existing metadata-only path.
fn resolve_dataset_header_addr_in_bytes(file_data: &[u8], path: &str) -> Result<u64, OxiH5Error> {
    let sb = superblock::parse(file_data)?;
    let root_msgs = header::parse_messages(file_data, sb.root_object_header_address)?;
    let normalized = path.trim_start_matches('/');
    let mut parts: Vec<&str> = normalized.split('/').filter(|s| !s.is_empty()).collect();
    ...
    if let Some((root_btree, root_heap)) = find_symbol_table_addresses(&root_msgs) {
        ...
        resolve_new_style_header_address(file_data, current_header, dataset_name, std::path::Path::new("."))
    }
}
